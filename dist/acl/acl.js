...
import { graph, serialize, st, sym } from 'rdflib';
 ...
 * synonym for "default".
 */
export function adoptACLDefault(doc, aclDoc, defaultResource, defaultACLDoc) {
    const ACL = ns.acl;
    ...
    function move(symbol) {
        ...
        return sym(symbol.uri.slice(0, y) === defaultACLDoc.uri
 ...
 */
export function readACL(doc, aclDoc, kb2 = kb, getDefaults = false) {
 ...
 */
export function loadUnionACL(subjectList, callbackFunction) {
    const aclList = [];
 ...
 */
export function ACLbyCombination(ac) {
    const byCombo = {};
    ['agent', 'agentClass', 'agentGroup', 'origin', 'originClass'].forEach(function (pred) {
 ...
 */
export function makeACLGraph(kb, x, ac, aclDoc) {
 ...
 */
export function makeACLGraphbyCombo(kb, x, byCombo, aclDoc, main, defa) {
    const ACL = ns.acl;
 ...
 */
export function makeACLString(x, ac, aclDoc) {
    const kb2 = graph();
 ...
 */
export function putACLObject(kb, x, ac, aclDoc, callbackFunction) {
    const byCombo = ACLbyCombination(ac);
 ...
 */
export function putACLbyCombo(kb, x, byCombo, aclDoc, callbackFunction) {
    var _a;
    const kb2 = graph();
    makeACLGraphbyCombo(kb2, x, byCombo, aclDoc, true);
    // const str = makeACLString = function(x, ac, aclDoc)
    (_a = kb.updater) === null || _a === void 0 ? void 0 : _a.put(aclDoc, kb2.statementsMatching(undefined, undefined, undefined, aclDoc), 'text/turtle', function (uri, ok, message) {
        var _a;
        if (!ok) {
            callbackFunction(ok, message);
        }
        else {
            (_a = kb.fetcher) === null || _a === void 0 ? void 0 : _a.unload(aclDoc);
 ...
 */
export function fixIndividualCardACL(person, log, callbackFunction) {
 ...
 */
export function fixIndividualACL(item, subjects, log, callbackFunction) {
    log = log || debug.log;
    const doc = item.doc();
 ...
 * Set an ACL
 */
export function setACL(docURI, aclText, callbackFunction) {
    const aclDoc = kb.any(docURI, ACL_LINK); // @@ check that this get set by web.js
    if (!kb.fetcher) {
 ...
 */
export function getACLorDefault(doc, callbackFunction) {
 ...
 */
export function getACL(doc, callbackFunction) {
    if (!kb.fetcher) {
    ...
    kb.fetcher.nowOrWhenFetched(doc, undefined, function (ok, body) {
        ...
        const aclDoc = kb.any(doc, ACL_LINK); // @@ check that this get set by web.js
        if (!aclDoc) {
 ...
 */
export async function getProspectiveHolder(targetDirectory) {
    return new Promise((resolve, reject) => getACLorDefault(sym(targetDirectory), (ok, isDirectACL, targetDoc, targetACLDoc, defaultHolder) => {
        if (ok) {
