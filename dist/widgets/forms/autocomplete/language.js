...
export async function getPreferredLanguagesFor(person) {
    var _a;
    const doc = person.doc();
    await ((_a = store.fetcher) === null || _a === void 0 ? void 0 : _a.load(doc));
    const list = store.any(person, ns.schema('knowsLanguage'), null, doc);
    if (!list) {
    ...
    list.elements.forEach(item => {
        ...
        const lang = store.any(item, ns.solid('publicId'), null, doc);
        if (!lang) {
