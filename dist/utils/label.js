...
export function label(thing, initialCap = false) {
    ...
    function cleanUp(label) {
        let result = '';
        if (label.slice(-1) === '/')
            label = label.slice(0, -1); // chop trailing slash
    ...
    if (typeof s === 'undefined')
        return thing.toString(); // can't be a symbol
    ...
    if (slash >= 0 && slash < thing.uri.length)
        return cleanUp(s.slice(slash + 1));
